...
public class XmfPlugin extends Plugin implements org.eclipse.ui.IStartup {
	...
	public String getImageDir() {
		URL location = XmfPlugin.class.getProtectionDomain().getCodeSource()
				.getLocation();
	...
	public Hashtable getImages() {
		String imagesDir = getEnvVar("XMFIMAGES");
		imagesDir = getImageDir();
		imagesDir += "Images";
		imagesDir = imagesDir.substring(6, imagesDir.length());
	...
	private String[] defaultImages = { "compiler", "eval", "mosaic", "server",
			"toolcompiler", "xmf" };

	private boolean isDefaultImage(String image) {
		boolean isDefault = false;
		for (int i = 0; i < defaultImages.length; i++) {
			if (image.equalsIgnoreCase(defaultImages[i] + ".img"))
				isDefault = true;
		}
		return isDefault;
	}
